...
sudo sed -i 's/worker_processes auto;/worker_processes '${PROCESSORS}';/g' /etc/nginx/nginx.conf
...
sudo chown -R ${USER}:www-data /home/www-data
...
sudo apt update && sudo apt upgrade && sudo apt install -y -q php8.0-fpm php8.0-common php8.0-mysql php8.0-xml php8.0-xmlrpc php8.0-curl php8.0-gd php8.0-imagick php8.0-cli php8.0-dev php8.0-imap php8.0-mbstring php8.0-opcache php8.0-soap php8.0-zip
...
sudo tar -xvf latest.tar.bz2 -C /home/www-data/
...
sudo adduser nextcloud www-data

#Setting Permissions
echo -e "\e[1;34mSetting permissions\e[0m"
sudo chown -R nextcloud:www-data /home/www-data/nextcloud
sudo chmod -R 770 /home/www-data
...
free -m
echo "-----------------------"
sudo systemctl start php8.0-fpm.service && ps --no-headers -o "rss,cmd" -C php-fpm8.0 | awk '{ sum+=$1 } END { printf ("%d%s\n", sum/NR/1024,"M") }'
echo "-----------------------"
...
sudo systemctl edit php8.0-fpm.service
sudo systemctl reenable php8.0-fpm.service
...
sudo mysql -u root -e "CREATE DATABASE IF NOT EXISTS ${newnextclouddbname} CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;"
echo -e "\e[1;32m${newnextclouddbname} database created.\e[0m"

sudo mysql -u root -e "CREATE USER IF NOT EXISTS "${newnextclouddbuser}"@"localhost";"
echo -e "\e[1;32m${newnextclouddbuser} user created.\e[0m"

sudo mysql -u root -e "SET password FOR "${newnextclouddbuser}"@"localhost" = password('${newnextclouddbpasswd}');"
echo -e "\e[1;32mPassword set.\e[0m"

sudo mysql -u root -e "GRANT ALL PRIVILEGES ON ${newnextclouddbname}.* TO "${newnextclouddbuser}"@"localhost" IDENTIFIED BY '${newnextclouddbpasswd}';"
...
sudo sed -i 's/443 ssl/443 ssl http2/g' /etc/nginx/sites-available/nextcloud.conf
...
sudo sed -i '/fastcgi_read_timeout      3600;/d' /etc/nginx/sites-available/nextcloud.conf

#Updating php8.0-fpm
echo -e "\e[1;34mUpdating php8.0-fpm (timeout)\e[0m"
sudo sed -i '/request_terminate_timeout = 3600/d' /etc/php/8.0/fpm/pool.d/nextcloud.conf


#Opcache
echo -e "\e[1;34mUpdating php8.0-fpm (opcache)\e[0m"
sudo sed -i 's/;opcache.enable=1/opcache.enable=1/g' /etc/php/8.0/fpm/php.ini
sudo sed -i 's/;opcache.enable_cli=0/opcache.enable_cli=1/g' /etc/php/8.0/fpm/php.ini
sudo sed -i 's/;opcache.memory_consumption=128/opcache.memory_consumption=128/g' /etc/php/8.0/fpm/php.ini
sudo sed -i 's/;opcache.interned_strings_buffer=8/opcache.interned_strings_buffer=32/g' /etc/php/8.0/fpm/php.ini
sudo sed -i 's/;opcache.max_accelerated_files=10000/opcache.max_accelerated_files=10000/g' /etc/php/8.0/fpm/php.ini
sudo sed -i 's/;opcache.revalidate_freq=2/opcache.revalidate_freq=1/g' /etc/php/8.0/fpm/php.ini
sudo sed -i 's/;opcache.save_comments=1/opcache.save_comments=1/g' /etc/php/8.0/fpm/php.ini



#Adding APCu and redis
sudo sed -i "s/'installed' => true,/'installed' => true,\n  'memcache.local' => '\\\OC\\\Memcache\\\APCu',\n  'memcache.locking' => '\\\OC\\\Memcache\\\Redis',\n  'redis' => array(\n    'host' => 'localhost',\n    'port' => 6379,\n  ),\nfail2ban/g" /home/www-data/nextcloud/config/config.php
sudo echo "apc.enable_cli=1" >> /etc/php/8.0/mods-available/apcu.ini
sudo systemctl restart php8.0-fpm.service

#Fail2Ban configuration
sudo mkdir /var/log/nextcloud
sudo chown nextcloud:nextcloud /var/log/nextcloud
sudo sed -i "s/fail2ban/  'loglevel' => 2,\n  'logtimezone' => 'Europe\/Paris',\n  'logfile' => '\/var\/log\/nextcloud\/nextcloud.log',\n  'log_rotate_size' => '104857600'/g" /home/www-data/nextcloud/config/config.php
...
sudo echo '#Postifix jail log' >> /var/log/mail.log
sudo echo '#Xinetd-fail log' >> /var/log/daemon.log
...
#patching database
sudo -u nextcloud php8.0 /home/www-data/nextcloud/occ db:add-missing-indices
sudo -u nextcloud php8.0 /home/www-data/nextcloud/occ db:convert-filecache-bigint
...
sudo sed -i "s/#Port 22/Port ${updatesshport}/g" /etc/ssh/sshd_config
...
sudo openssl dhparam -out /etc/ssl/certs/dhparam.pem 4096
sudo chmod 600 /etc/ssl/certs/dhparam.pem
sudo sed -i 's/ssl_dhparam \/etc\/letsencrypt\/ssl-dhparams.pem; # managed by Certbot/ssl_dhparam \/etc\/ssl\/certs\/dhparam.pem;/g'  /etc/nginx/sites-available/nextcloud.conf
